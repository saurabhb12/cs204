...
lli med(lli arr[],lli l,lli r,lli k)
{
    if (k > 0 && k <= r - l + 1)
    {
        lli n = r - l + 1;
        ...
        lli medOfMed = (i == 1) ? median[i - 1] : med(median, 0, i - 1, i / 2);
        lli pos = partition(arr, l, r, medOfMed);
        if (pos - l == k - 1)
            return arr[pos];
        if (pos - l > k - 1)
            return med(arr, l, pos - 1, k);

        return med(arr, pos + 1, r, k - pos + l - 1);
    }
    return INT_MAX;
}
